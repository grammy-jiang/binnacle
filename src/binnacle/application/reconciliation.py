...
class AuditRecoveryService:
    ...
    async def recover(
        ...
        if not latched or generation != active_generation or recovered_generation >= generation:
        ...
        if not markers:
            ...
            if existing_recovery is None:
                raise RuntimeError("exact-generation recovery evidence is absent")
        ...
        if len(supplied) != len(closures) or set(supplied) != set(by_id):
        ...
        for closure in closures:
            try:
                digest_valid = (
                    len(closure.evidence_sha256) == 64
                    and int(closure.evidence_sha256, 16) >= 0
                    and closure.evidence_sha256 == closure.evidence_sha256.casefold()
                )
            except ValueError:
                digest_valid = False
            if closure.effect_outcome not in outcome_knowledge or not digest_valid:
                raise RuntimeError("audit obligation closure evidence is invalid")
            marker = by_id[closure.obligation_id]
            ...
            if operation.effect_knowledge is not outcome_knowledge[closure.effect_outcome]:
            ...
            if closure.evidence_sha256 != expected_evidence:
            ...
            closure_hashes.append(result.event_hash)
        ...
        verification = await self._audit.append(
            AuditEventDraft(
                event_id=f"event_{secrets.token_hex(16)}",
                recorded_at=datetime.now(UTC),
                monotonic_ns=time.monotonic_ns(),
        ...
        recovery_digest = hashlib.sha256(
            ...
            + b"\0"
            + "\0".join(closure_hashes).encode()
            + b"\0"
            + verification.event_hash.encode()
        ).hexdigest()
        completed = await self._audit.append(
            AuditEventDraft(
                ...
                severity="notice",
                source="local_owner",
                correlation_ids=tuple(sorted(by_id)),
                payload={
                    "kind": "recovery.completed",
...
class OperationReconciler:
    ...
    async def reconcile_startup(self) -> tuple[OperationSnapshot, ...]:
        ...
        if not obligations and not latched and generation == recovered:
