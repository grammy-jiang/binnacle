"""Socket, DNS, TCP, composition, and failure-path tests for uplink."""
...
def test_sudo_timeout_argv_leaves_non_sudo_and_unknown_flags_alone():
    assert uplink.sudo_timeout_argv([], 2.5) == []
    assert uplink.sudo_timeout_argv(["ip", "route"], 2.5) == ["ip", "route"]
    assert uplink.sudo_timeout_argv(["sudo", "-u", "root", "id"], 2.5) == [
        "sudo",
        "-u",
        "root",
        "id",
...
def test_sudo_timeout_argv_handles_empty_already_wrapped_and_floor():
    assert uplink.sudo_timeout_argv(["sudo", "-n"], 2.5) == ["sudo", "-n"]
    assert uplink.sudo_timeout_argv(
        ["sudo", "-n", "timeout", "5", "iw", "dev"], 2.5
    ) == ["sudo", "-n", "timeout", "5", "iw", "dev"]
    assert uplink.sudo_timeout_argv(["sudo", "-n", "iw", "dev"], 0.2) == [
        "sudo",
        "-n",
        "timeout",
        "1",
...
def test_first_a_record_rejects_truncated_answer_header():
    # One question, one answer; answer name is present but the RR header is not.
    query = (
        b"\x12\x34\x81\x80"
    ...
    assert uplink._first_a_record(query) is None


def test_skip_name_returns_end_for_unterminated_label_sequence():
    assert uplink._skip_name(b"\x03abc", 0) == 4
